...
class ZhipuClient:
    ...
    async def create_embeddings(
        ...
        Args:
            ...
            dimensions: Vector dimensions (256, 512, 1024, or 2048)
            
        Returns:
            List of embedding vectors
            
        Raises:
            ZhipuError: If API call fails
        """
        if not self.api_key:
            raise ZhipuError("Zhipu API key not configured")
        
        model = model or self.embedding_model
        dimensions = dimensions or self.embedding_dimensions
        
        # Truncate texts if too long (embedding-3 supports 2048 tokens)
        processed_texts = []
        for text in texts:
            if len(text) > 8000:  # Rough estimate: 2048 tokens ~ 6000-8000 chars
                text = text[:8000]
            processed_texts.append(text)
        
        logger.info(
            "Creating embeddings with Zhipu",
            model=model,
        ...
        try:
            ...
            raise ZhipuError(f"Zhipu API error: {e.response.status_code} - {e.response.text}")
        except Exception as e:
            logger.error("Zhipu API error", error=str(e))
            raise ZhipuError(f"Failed to create embeddings: {e}")
    ...
    async def chat_completion(
        ...
        try:
            ...
            raise ZhipuError(f"Zhipu API error: {e.response.status_code}")
        except Exception as e:
            logger.error("Zhipu API error", error=str(e))
            raise ZhipuError(f"Failed to create chat completion: {e}")
...
class PatentEmbedder:
    ...
    def __init__(self, zhipu_client: Optional[ZhipuClient] = None):
        self.zhipu = zhipu_client or ZhipuClient()
    ...
    async def embed_patent(
        ...
        result["title_abstract"] = await self.zhipu.create_embedding(title_abstract)
        
        # Embed claims
        claims_text = "\n".join([f"权利要求{i+1}：{c}" for i, c in enumerate(claims[:10])])
        result["claims"] = await self.zhipu.create_embedding(claims_text)
        
        # Embed description (chunked if long)
        if description:
            chunks = self._chunk_text(description)
            chunk_embeddings = await self.zhipu.create_embeddings(chunks)
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        if len(chunks) == 1:
            return await self.zhipu.create_embedding(chunks[0])
        else:
